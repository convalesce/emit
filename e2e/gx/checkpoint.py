...
listed the way the docs say, on whichever GX major is installed."""
...
FRAME = pd.DataFrame(
    {
        "email": ["alice@example.com", "bob@example.com", None, "dora@example.com"],
        "amount": [10, 20, 30, 40],
    }
)
...
def run_v1():
    ...
    definition = context.validation_definitions.add(
        gx.ValidationDefinition(name="orders", data=batch_def, suite=suite)
    )
...
def run_v0():
    ...
    validator = context.get_validator(batch_request=request, expectation_suite_name="orders")
    validator.expect_column_values_to_not_be_null("email")
    validator.save_expectation_suite(discard_failed_expectations=False)
    checkpoint = context.add_or_update_checkpoint(
        name="orders",
        validations=[{"batch_request": request, "expectation_suite_name": "orders"}],
        action_list=[
            {
                "name": "convalesce",
                "action": {
